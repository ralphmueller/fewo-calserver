...
def income_year():
    ...
    return render_template('income_year_by_apartment.html', data=data, run_mode=bkormlib.envir)
'''

@bp.route('/income_and_commission')
def income_and_commission():
    return render_template('apartment_income_and_commission.html', 
        sel='',
...
def rest_apartment_income_and_commission(apartment_name):
    res = {}
    res['datasets'] = calc_apartment_income_and_commission_for_years(apartment_name, YEARS)
...
def abgerechnete_besuche():
    return render_template('abgerechnete-besuche.html', data=aggregate_byMonth_byApartment(), run_mode=envir)
